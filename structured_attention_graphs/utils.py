...
def load_model_new(use_cuda=1, model_name='resnet50'):
    ...
    try:
        if model_name in model_name_dict:
            model = timm.create_model(model_name_dict[model_name], pretrained=True)
        else:
            model = timm.create_model(model_name, pretrained=True)
    except Exception as e:
        raise ValueError(f"Model {model_name} not found or could not be loaded: {e}")
